...
use_cmd = {
    ...
    'aurman': [
        'aurman', '-S', '--noconfirm', '--noedit', '--needed', '--skip_news',
        '--pgp_fetch', '--skip_new_locations'
    ],
...
def check_packages(module, packages):
    ...
    if would_be_changed:
        status = True
        if (len(packages) > 1):
            message = '%s package(s) would be installed' % str(
...
def install_with_makepkg(module, package):
    ...
    f = open_url(
        'https://aur.archlinux.org/rpc/?v=5&type=info&arg={}'.format(package))
...
def upgrade(module, use, aur_only):
    ...
    rc, out, err = module.run_command(
        def_lang + use_cmd[use] +
        ['--aur' if (aur_only and use in has_aur_option) else None] + ['-u'],
        check_rc=True)

    module.exit_json(
        changed=not (out == '' or 'nothing to do' in out
                     or 'No AUR updates found' in out),
        msg='upgraded system',
        helper=use,
    )
    ...
    for package in packages:
        ...
        if use == 'makepkg':
            ...
            rc, out, err = module.run_command(
                def_lang + use_cmd[use] +
                ['--aur' if
                 (aur_only and use in has_aur_option) else None] + [package],
                check_rc=True)
...
def main():
    module = AnsibleModule(
        argument_spec={
            ...
            'use': {
                ...
                'auto',
                'choices': [
                    'auto', 'aurman', 'pacaur', 'trizen', 'pikaur', 'yaourt',
                    'yay', 'makepkg'
                ],
            },
            'skip_installed': {
    ...
    if params['upgrade'] and (params['name'] or params['skip_installed']
                              or use == 'makepkg'):
        ...
        if params['upgrade']:
            ...
            install_packages(module, params['name'], use,
                             params['skip_installed'], params['aur_only'])
